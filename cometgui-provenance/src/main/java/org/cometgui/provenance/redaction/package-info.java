 ...
 *
 * <p>So there is one object, {@link org.cometgui.provenance.redaction.SecretRedactor}, and every
 * writer in this module calls it. Adding a field to the JSON manifest, a row to the
 * reStructuredText report or a line to the event log cannot open a new leak path, because the new
 * field goes through the same {@code redactText} the old ones do.
 *
 ...
 * {@link org.cometgui.provenance.redaction.SecretRegistry}, is the airtight half: the application
 * knows the credential it is holding, because it read it out of the OS keychain, so it registers
 * that exact value and every emitted string has that substring replaced wherever it appears.
 * Pattern rules alone cannot catch a bare token passed as {@code -k <value>}; the registry can, and
 * that is what makes phase 04's exit gate item 6 an honest claim rather than a hopeful one.
 *

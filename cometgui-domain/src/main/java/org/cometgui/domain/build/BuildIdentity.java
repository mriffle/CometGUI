...
/**
 * Immutable identity of the running build: application version, source commit and build
 * timestamp.
 *
 ...
 * manifest. The properties it parses are the ones Maven resource filtering will produce.</p>
 */
public final class BuildIdentity {
    ...
    public static BuildIdentity of(String version, String commitId, Instant buildTimestamp) {
        ...
        if (commitId == null
                || !(UNKNOWN_COMMIT.equals(commitId) || FULL_COMMIT_ID.matcher(commitId).matches())) {
            throw new IllegalArgumentException(
                    "build commit id must be 40 lowercase hex characters or \"" + UNKNOWN_COMMIT
                            + "\", but was: " + commitId);
        }
    ...
    public static BuildIdentity fromProperties(Properties properties) {
        ...
        try {
            ...
            throw new IllegalArgumentException(
                    "build property " + TIMESTAMP_KEY + " is not an ISO-8601 instant: " + rawTimestamp,
                    e);
        }
        return of(properties.getProperty(VERSION_KEY), properties.getProperty(COMMIT_KEY), timestamp);
    }
    ...
    public String toString() {
        return "BuildIdentity[version=" + version + ", commit=" + commitId + ", built=" + buildTimestamp + "]";
    }
}
